...
TARGET_URL = 'https://ggoorr.net/info'
...
def fetch_article_content(full_url):
    try:
        ...
        return content_html, ['hotdeals']
    except Exception as e:
        return f"<p>Error fetching content: {e}</p>", ['hotdeals']
...
def generate_rss(items):
    fg = FeedGenerator()
    fg.title('Ggoorr hotdeals Feed')
    fg.link(href=BASE_URL)
    fg.description('Latest hotdeals posts from Ggoorr')
    fg.language('ko')
    ...
    fg.rss_file('gorhotdeals.xml')
    print("✅ RSS feed created: gorhotdeals.xml")
