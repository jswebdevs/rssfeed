...
TARGET_URL = 'https://www.etoland.co.kr/bbs/board.php?bo_table=sisabbs01'
...
def fetch_article_content(full_url):
    try:
        ...
        if not content_div:
            return "<p>No content found</p>", ['review']
        ...
        return content_html, ['review']
    except Exception as e:
        return f"<p>Error fetching content: {e}</p>", ['review']
...
def generate_rss(items):
    fg = FeedGenerator()
    fg.title('Eto Review Feed')
    fg.link(href=BASE_URL)
    fg.description('Latest review posts from etoland')
    fg.language('ko')
    ...
    fg.rss_file('etoreview.xml')
    print("✅ RSS feed created: etoreview.xml")
