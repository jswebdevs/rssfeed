...
def generate_rss_feed(items, output_file=FEED_FILE):
    ...
    etree.SubElement(channel, "link").text = "https://zdnet.co.kr/"
    etree.SubElement(channel, "description").text = "RSS feed generated from zdnet.co.kr"
    ...
    for idx, item in enumerate(items):
        ...
        if item_type == 'article':
            ...
            link = (item.get('link') or '').strip() or f"https://zdnet.co.kr/placeholder/{idx + 1}"
            ...
            link = item.get('src') or f"https://zdnet.co.kr/video/{idx + 1}"
